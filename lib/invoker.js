...
function knownError(test, act) {
  ...
  else if (toString.call(test) === '[object RegExp]') {
    test = function (input) {
  ...
  if (typeof act === 'number') {
    ...
    act = function (res) {
      throw new Exc(res.message, res.exception);
    };
  }
...
function getBody(res) {
  var json = (res.type === 'text/javascript') || (res.type === 'application/json');
  if (res.text === 'false' && json) return false;
...
function parseLiferayResponse(res) {
  ...
  invoker.knownErrors.some(function (el) {
    if (el.test(exception) || el.test(message)) {
      el.act(res);
      return true;
    }
  });
